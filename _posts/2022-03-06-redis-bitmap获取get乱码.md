...
&emsp;需要记录学生听课记录时间精确到秒，在讨论过程中采用了redis的bitmap存储学员听课记录。利用bitmap的结构存储学员的听课记录，例如一个视频的大小为1小时，拆解下来就是60分钟，拆成秒的话是3600秒，对应创建一个长度为3600的bitmap结构存储听课记录，客户端时时上报。并且也能检测到学员哪一秒看了<br/>
